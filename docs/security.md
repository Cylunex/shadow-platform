...
- Health 的既有 Forward Auth 链路仍须同时校验真实传输对端、应用独立代理密钥和准入组；
  该例外不得复制到新项目。
- Shadow Nexus 是嵌入无原生认证能力的 DSH Web 的托管界面，只允许在独立公网域名外围使用
  Authelia Forward Auth 作为访问门禁。Nexus 和领域服务不得使用这些代理身份头授权业务操作；
  领域调用仍使用各项目独立 Agent Bearer。DSH 上游必须只监听回环地址。
